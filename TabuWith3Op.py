...
def store_best_relocation_move( originRouteIndex, targetRouteIndex, originNodeIndex, targetNodeIndex,
                            moveCost, rm: RelocationMove):
...
def apply_relocation_move(rm, sol: Solution, tabu: Tabu, iteration,matrix):
    ...
    tabu.set_tabu(targetRt.sequenceOfNodes[rm.targetNodePosition], iteration)
...
def find_best_relocation_move(rm,sol: Solution, best_sol: Solution, iteration, matrix):
                    ...
                    for targetNodeIndex in range(0, len(rt2.sequenceOfNodes)):
                        if rt1 != rt2:
                            ...
                            if current_cap + rt1.sequenceOfNodes[originNodeIndex].demand > rt2.capacity:
                                continue
                            moveCost = calculate_inter_relocation_cumulative_change(originNodeIndex,targetNodeIndex,rt1,rt2,matrix)
                        else:
                            moveCost = calculate_intra_relocation_cumulative_change(originNodeIndex,targetNodeIndex,rt1,matrix)
                        if Tabu.move_is_tabu(sol, best_sol, rt1.sequenceOfNodes[originNodeIndex], moveCost,
                                         iteration) or \
                            Tabu.move_is_tabu(sol, best_sol, rt2.sequenceOfNodes[targetNodeIndex], moveCost,
                                              iteration):
                            continue
                        if (moveCost < rm.moveCost):
                            store_best_relocation_move(originRouteIndex, targetRouteIndex, originNodeIndex,
...
def tabu_search(initial_solution, tenure_range, iterations, matrix, depot, capacity,):
    ...
    for iterator in range(iterations):
        ...
        rm.Initialize()
        if  operator == 2 :
            find_best_relocation_move(rm, solution, best_solution, iterator, matrix)
            if rm.originRoutePosition is not None :#and sm.moveCost < 0:
                apply_relocation_move(rm, solution, tabu, iterator,matrix)
...
def main():
    ...
    #initial_best_solution = calculate_initial_best_solution(population, nodes, depot, capacity,number_of_vehicles, distance_matrix)
