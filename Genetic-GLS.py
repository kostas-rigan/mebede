...
def penalize_arcs(sol : Solution, matrix, pen : Penalize):
        ...
        #     matrix_penalized[self.penalized_n1_ID][self.penalized_n2_ID] = matrix[self.penalized_n1_ID][self.penalized_n2_ID]
        #     matrix_penalized[self.penalized_n2_ID][self.penalized_n1_ID] = matrix[self.penalized_n2_ID][self.penalized_n1_ID]
        max_criterion = 0
        pen_1 = -1
        pen_2 = -1
...
def local_search(initial_solution, depot, capacity, matrix, pen: Penalize, number_of_iterations):
    ...
    while terminationCondition is False:
            # epeidi o relocation einai under construction DEN ton exo sto randint
...
def main():
    ...
    # allh mia gia kati poy thelei o relocation 
    distance_matrix = calculate_cost_matrix([depot] + nodes,len([depot] + nodes)+1)
    rows = len(nodes) + 2
    pen = Penalize(rows,distance_matrix)

    number_of_iterations = 5000
    for i in range(len(initial_best_solution.routes)):
        calculate_pen_cum_matrix(initial_best_solution.routes[i],pen)
